...
- **DEP-003**: **PostgreSQL 16.11 with pgvector Extension** - Core data store. Must be available before Phase 1 development. Mitigation: Docker Compose configuration provides easy setup.
...
- **DEP-009**: **Microsoft.SemanticKernel 1.70.0** - LLM orchestration framework. Required in Phase 2 TASK-034.
...
- **DEP-013**: **FluentValidation 12.1.1** - Input validation library. Required from Phase 1.

- **DEP-014**: **Polly 8.6.5** - Resilience and fault handling. Required in Phase 2 for LLM API calls.

- **DEP-015**: **prometheus-net** - Metrics collection library. Required in Phase 4.

### Testing Dependencies

- **DEP-016**: **Testcontainers 4.10.0** - Integration testing with Docker containers. Required from Phase 1 for database integration tests.

- **DEP-017**: **xUnit v3, FluentAssertions 8.8.0, Moq 4.20.72** - Unit testing frameworks. Required from Phase 1.
...
- **FILE-037**: `src/InfoDumpManager.Infrastructure/Services/OpenAILLMProvider.cs` - OpenAI provider implementation using Azure.AI.OpenAI 2.1.0
- **FILE-038**: `src/InfoDumpManager.Infrastructure/Services/AzureOpenAILLMProvider.cs` - Azure OpenAI provider implementation using Azure.AI.OpenAI 2.1.0
- **FILE-039**: `src/InfoDumpManager.Infrastructure/Services/LLMOrchestrationService.cs` - LLM orchestration with Microsoft.SemanticKernel 1.70.0
...
- **TEST-001**: **Domain Entity Validation Tests** - Test all domain entities for invariant enforcement, value object immutability, and business rule validation using xUnit v3 and FluentAssertions 8.8.0. Target: 90%+ coverage of domain layer.

- **TEST-002**: **Command Handler Tests** - Test all MediatR 14.0.0 command handlers with mocked dependencies (repositories, services) to verify business logic, validation, and error handling. Use Moq 4.20.72 for mocking.
...
- **TEST-004**: **Validator Tests** - Test all FluentValidation 12.1.1 validators with valid and invalid inputs to ensure comprehensive input validation coverage.
...
- **TEST-007**: **Database Integration Tests** - Use Testcontainers 4.10.0 to spin up PostgreSQL 16.11 instance and test repository implementations, migrations, and complex queries. Verify data persistence, retrieval, and concurrency handling.
